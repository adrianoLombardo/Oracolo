...
from .startup import StartupData
from .audio_setup import AudioSetup

logger = logging.getLogger(__name__)


def run(start: StartupData, audio: AudioSetup) -> None:
    """Main run loop handling the session state machine."""
    SET = start.settings
    args = start.args
    DEBUG = start.debug
    tone = start.tone
    client = start.client
    cfg_path = start.cfg_path
    session_profile_name = start.session_profile_name
    session_persona_name = start.session_persona_name
    session_id = start.session_id
    listener = start.listener

    AUDIO_CONF = audio.audio_conf
    AUDIO_SR = audio.audio_sr
    INPUT_WAV = audio.input_wav
    OUTPUT_WAV = audio.output_wav
    PREPROC = audio.preproc
    in_dev = audio.in_dev
    out_dev = audio.out_dev
    recording_conf = audio.recording_conf
    vad_conf = audio.vad_conf
    lighting_conf = audio.lighting_conf
    light = audio.light
    is_tts_playing = audio.is_tts_playing
        ...
        while True:
            ...
            if dlg.state == DialogState.LISTENING:
                ...
                if m:
                    ...
                    if target:
                        ...
                        try:
                            ...
                            pass
                        pending_answer = f"Personalità cambiata in {personas.get(target, {}).get('label', target)}."
                    else:
                ...
                ok, context, clarify, reason, suggestion = validate_question(
                    ...
                    model=EMB_MODEL,
                    client=client,
                    threshold=SET.domain.validator_threshold,
                    cfg=SET.domain.model_dump(),
                )
                if not ok:
                    if clarify:
                        say(clarify)
                    ...
                    if reason:
                        say(reason)
                        continue
                topic = SET.domain.topic
                pending_q = q
                pending_lang = session_lang or "it"
                if context:
                    pending_history = dlg.history(maxlen=CHAT_HISTORY_LIMIT) if CHAT_ENABLED else None
                    pending_topic = topic
                else:
                    pending_history = None
                    pending_topic = None
                processing_turn = dlg.turn_id
                dlg.begin_processing()
                dlg.transition(DialogState.PROCESSING)
                continue

            # ---------------------- PROCESSING: prepara risposta ---------------------- #
            if dlg.state == DialogState.PROCESSING:
                if dlg.turn_id != processing_turn:
                    dlg.transition(DialogState.LISTENING)
                    continue
                if matches_hotword_text(pending_q, WAKE_IT + WAKE_EN):
                    pending_answer = default_response("wake", session_lang or "it", tone)
                    pending_full_answer = pending_answer
                    dlg.transition(DialogState.SPEAKING)
                    continue
                context_texts = None
                ...
                domain_set = make_domain_settings({}, SET.domain.profile, {"topic": SET.domain.topic})
                profile_hint = domain_set["domain"].get("profile_hint", "")
                effective_system = (
